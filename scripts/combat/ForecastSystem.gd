...
static func build_forecast(attacker: UnitModel, defender: UnitModel, map: MapModel) -> Dictionary:
    var attacker_weapon: Dictionary = Registries.weapons.get_weapon(attacker.weapon_id)
    ...
    var terrain_label: String = TerrainSystem.get_terrain_label(TerrainSystem.get_tile_data(map, defender.position))
    return {
