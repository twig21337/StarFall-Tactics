...
static func compute_combat_stats(attacker: UnitModel, defender: UnitModel, map: MapModel) -> Dictionary:
    var weapon: Dictionary = Registries.weapons.get_weapon(attacker.weapon_id)
    var defender_weapon: Dictionary = Registries.weapons.get_weapon(defender.weapon_id)
    var triangle: Dictionary = TriangleSystem.get_triangle(weapon, defender_weapon)
    var tile: Dictionary = TerrainSystem.get_tile_data(map, defender.position)
    var atk := 0
    ...
    var crit_avo: int = defender.get_stat("lck")
