...
static func take_turn(state: BattleState, unit: UnitModel) -> Dictionary:
    var weapon: Dictionary = Registries.weapons.get_weapon(unit.weapon_id)
    var enemies: Array = state.get_units_by_side("PLAYER")
    var occupied: Dictionary = _build_occupancy(state.units, unit)
    var reachable: Array = MovementSystem.get_reachable_tiles(state.map, unit.position, unit.get_stat("mov"), occupied)
    reachable.append(unit.position)
    var targets_now: Array = TargetingSystem.get_targets_in_range(unit, enemies, weapon)
    ...
    var best_target: UnitModel = _closest_unit(unit.position, enemies)
